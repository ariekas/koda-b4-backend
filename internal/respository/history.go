...
LEFT JOIN products p ON ti.products_id = p.id
LEFT JOIN (
    SELECT DISTINCT ON (products_id) products_id, image
    FROM product_images
    ORDER BY products_id, id ASC
) pi ON pi.products_id = p.id
LEFT JOIN size_products sp ON ti.product_sizes_id = sp.id
LEFT JOIN variant_products vp ON ti.product_variants_id = vp.id
LEFT JOIN deliverys d ON t.deliverys_id = d.id
WHERE t.id = $1 
  AND t.users_id = $2
GROUP BY 
    t.id, t.invoice_num, t.name_user, t.address_user, t.phone_user, 
    pm.name, d.name, st.status, t.total;
`, historyId, userID).Scan(&historys.Id,
		&historys.Invoice,
		&historys.Fullname,
		&historys.Address,
		&historys.Phone,
